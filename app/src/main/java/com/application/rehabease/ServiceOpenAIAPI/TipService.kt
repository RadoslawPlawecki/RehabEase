...
object TipService {
    ...
    private val exercisePrompts = listOf(
    ...
    private val nutritionPrompts = listOf(
    ...
    private val sleepPrompts = listOf(
    ...
    private val mentalHealthPrompts = listOf(
    ...
    private val hydrationPrompts = listOf(
    ...
    private val posturePrompts = listOf(
    ...
    private val selfCarePrompts = listOf(
    ...
    private val cookingPrompts = listOf(
    ...
    val prompts = mapOf(
        "exercise" to exercisePrompts,
        "nutrition" to nutritionPrompts,
        "sleep" to sleepPrompts,
        "mental_health" to mentalHealthPrompts,
        "hydration" to hydrationPrompts,
        "posture" to posturePrompts,
        "self_care" to selfCarePrompts,
        "cooking" to cookingPrompts
    )

    suspend fun getTipOfTheDay(category: String): String {
        val prompt = getRandomPromptFromCategory(category)
        return fetchTipFromOpenAI(prompt)
    }

    private fun getRandomPromptFromCategory(category: String): String {
        val categoryPrompts = prompts[category] ?: error("Invalid category: $category")
        return categoryPrompts[Random.nextInt(categoryPrompts.size)]
    }
        ...
        try {
            ...
            if (response.isSuccessful && responseBody != null) {
                ...
                if (choicesArray.length() > 0) {
                    ...
                    return@withContext "No choices found in the response."
                }
            } else {
            ...
            return@withContext "IOException: ${e.message}"
        }
    }
}